...
Every Resend CLI command must follow this pattern to ensure the PATH is set and the API key is loaded securely:
...
**Security**: Never hardcode, log, or display the full API key. Always read it dynamically from the project's environment file.
...
When a profile is active, it takes precedence over env-vars unless `--api-key` is passed explicitly.
...
The CLI has 13 command groups covering all Resend functionality. For exact flags and syntax, read `references/commands.md`.
...
6. **Pagination** — Most list commands default to 10 results. Use `--limit` (max 100) and `--after`/`--before` cursors to page through larger datasets. Check `has_more` in the response.
